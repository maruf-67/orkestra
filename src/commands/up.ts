...
export async function up(options: UpOptions) {
  ...
  // Laravel: run composer dev directly (project's own script handles concurrently)
  if (framework.name === "laravel" && execCmd === "composer" && execArgs.includes("dev")) {
    execCmd = "composer";
    execArgs = ["dev"];
  }
  ...
  // Clean up any stale Laravel Octane / RoadRunner processes
  if (framework.name === "laravel") {
    await cleanupLaravelProcesses(projectDir, port);
  }
  ...
  const env: Record<string, string | undefined> = {
    ...process.env,
    PORT: String(port),
    SERVER_PORT: String(port), // Laravel composer dev uses this
  };
